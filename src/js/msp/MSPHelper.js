...
function MspHelper () {
  ...
  // 0 based index, must be identical to 'baudRates' in 'src/main/io/serial.c' in butterflight
  ...
  // needs to be identical to 'serialPortFunction_e' in 'src/main/io/serial.h' in butterflight
    ...
    if (!crcError) {
        if (!dataHandler.unsupported) switch (code) {
            ...
            case MSPCodes.MSP_IMUF_CONFIG:
                console.log('IMUFFF')
                KALMAN_FILTER_CONFIG.imuf_mode = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_pitch_q = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_pitch_w = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_roll_q = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_roll_w = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_yaw_q = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_yaw_w = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_pitch_lpf_cutoff_hz = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_roll_lpf_cutoff_hz = data.readU16();
                KALMAN_FILTER_CONFIG.imuf_yaw_lpf_cutoff_hz = data.readU16();
                break;
...
MspHelper.prototype.crunch = function(code) {
    ...
    switch (code) {
        ...
        case MSPCodes.MSP_SET_IMUF_CONFIG: 
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_mode);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_pitch_q);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_pitch_w);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_roll_q);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_roll_w);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_yaw_q);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_yaw_w);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_pitch_lpf_cutoff_hz);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_roll_lpf_cutoff_hz);
            buffer.push16(KALMAN_FILTER_CONFIG.imuf_yaw_lpf_cutoff_hz);
            break;
        case MSPCodes.MSP_SET_PID_ADVANCED:
            if (semver.gte(CONFIG.apiVersion, "1.20.0")) {
